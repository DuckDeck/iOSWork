...
class CityTableViewCell: UITableViewCell {
        ...
        didSet {
            if let cities = info {
                ...
                while i < cities.count {
                    ...
                    let btn = UIButton(frame: f).title(title: cities[i].city).color(color: UIColor.Hex(hexString: "#f64950")).bgColor(color: UIColor.white).cornerRadius(radius: 4).borderWidth(width: 1).borderColor(color: UIColor.Hex(hexString: "#f64950"))
    ...
    convenience init(style: UITableViewCell.CellStyle, reuseIdentifier: String?,cities:[(String,Int)]) {
        ...
        while i < cities.count {
            let f = CGRect(x: CGFloat(i % 3) * (btnWidth + 10) + 15, y: 10 + CGFloat( i / 3) * (btnHeight + 10) , width: btnWidth, height: btnHeight)
            let btn = UIButton(frame: f).title(title: cities[i].0).color(color: UIColor.Hex(hexString: "#f64950")).bgColor(color: UIColor.white).cornerRadius(radius: 4).borderWidth(width: 1).borderColor(color: UIColor.Hex(hexString: "#f64950"))
            btn.tag = cities[i].1
            btn.titleLabel?.font = UIFont.systemFont(ofSize: 14)
            btn.addTarget(self, action: #selector(CityTableViewCell.btnClick(sender:)), for: .touchUpInside)
            addSubview(btn)
