...
+ (void)logWithLevel:(TLogLevel)logLevel moduleName:(const char*)moduleName fileName:(const char*)fileName lineNumber:(int)lineNumber funcName:(const char*)funcName message:(NSString *)message {
    if (![SwitchManager getSwitchWithKey:SwitchManager.switch_xlog]) {
        CCLog([NSString stringWithUTF8String:moduleName],message);
        return;
    }
    if (message.length > 15000) { // 日志太长，需要关联
        NSDate *currentDate = [NSDate date];
        NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
        [dateFormatter setDateFormat:@"yyyy-MM-dd HH:mm:ss.SSS"];
        NSString *connectId = [@"关联类型-" stringByAppendingString:[dateFormatter stringFromDate:currentDate]];
        CCLog(connectId,message);
        message = connectId;
        moduleName = "关联日志";
    }
    XLoggerInfo info;
